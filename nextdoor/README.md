...
This printed CLI provides a non-interactive token workflow for Nextdoor Ads.

## Install

Run go build ./... from this directory. The binary is nextdoor-pp-cli.
...
Create or approve an app in the official developer console. Follow the platform's access steps in the links below. Then run:
    ...
    nextdoor-pp-cli auth set-token YOUR_ACCESS_TOKEN

You can also set NEXTDOOR_ADS_ACCESS_TOKEN. Check the local setup with:

    nextdoor-pp-cli doctor
