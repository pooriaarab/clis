...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLARITY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLARITY_FEEDBACK_AUTO_SEND")))
...
When ` + "`CLARITY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLARITY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
