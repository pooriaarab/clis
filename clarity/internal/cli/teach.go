...
// clarity-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"clarity-pp-cli/internal/learn/entities"
	"clarity-pp-cli/internal/learn/lookups"
	"clarity-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "CLARITY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("clarity-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  clarity-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  clarity-pp-cli recall "<question>" --agent
  clarity-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clarity-pp-cli learnings list --agent
  clarity-pp-cli learnings list --query "<substring>"
  clarity-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  clarity-pp-cli learnings forget "<question>" --resource <id>
  clarity-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  clarity-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  clarity-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
