...
Run sync first to populate the local search index.`,
  ...
  clarity-pp-cli search "status"

  # Force local search explicitly
  clarity-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  clarity-pp-cli search "status" --type export-data --data-source local
  # JSON output for piping
  clarity-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clarity-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
