...
type Config struct {
	...
	CineplexSubscriptionKey string          `toml:"subscription_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://apis.cineplex.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("CINEPLEX_SUBSCRIPTION_KEY"); v != "" {
		cfg.CineplexSubscriptionKey = v
		cfg.markEnvOverride("CineplexSubscriptionKey")
		cfg.AuthSource = "env:CINEPLEX_SUBSCRIPTION_KEY"
		cfg.CredentialSource = "env:CINEPLEX_SUBSCRIPTION_KEY"
	}
	...
	if v := os.Getenv("CINEPLEX_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CINEPLEX_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "cineplex-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.CineplexSubscriptionKey
	if token == "" {
		return ""
	}
	if c.CineplexSubscriptionKey == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                     token,
		"subscription_key":          c.CineplexSubscriptionKey,
		"CINEPLEX_SUBSCRIPTION_KEY": c.CineplexSubscriptionKey,
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		CineplexSubscriptionKey: c.CineplexSubscriptionKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.CineplexSubscriptionKey = creds.CineplexSubscriptionKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.CineplexSubscriptionKey = token
	delete(c.envOverrides, "CineplexSubscriptionKey")
	c.updateFileConfigField("CineplexSubscriptionKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CineplexSubscriptionKey"] {
			out.CineplexSubscriptionKey = c.fileConfig.CineplexSubscriptionKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CineplexSubscriptionKey":
		c.fileConfig.CineplexSubscriptionKey = c.CineplexSubscriptionKey
	}
}
