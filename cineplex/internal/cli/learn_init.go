...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{3,6}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: cineplex-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{3,6}$`, err)
	} else {
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: cineplex-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: cineplex-pp-cli: learn init: %v\n", err)
		}
	})
}
