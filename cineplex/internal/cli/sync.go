...
import (
	...
	"cineplex-pp-cli/internal/learn/lookups"
...
Resource scoping:
  ...
  cineplex-pp-cli sync

  # Sync specific resources only
  cineplex-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cineplex-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cineplex-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cineplex-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cineplex-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("cineplex-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "cineplex-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "concessions":
		return db.UpsertConcessions(data)
	case "movies":
	...
	case "scene":
		return db.UpsertScene(data)
	case "seats":
		return db.UpsertSeats(data)
	case "showtimes":
		return db.UpsertShowtimes(data)
	case "theatres":
		return db.UpsertTheatres(data)
	case "utilities":
		return db.UpsertUtilities(data)
	default:
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"movies",
		"scene",
		"scene-get-payment-cards",
		"showtimes",
		"theatres",
		"theatres-playingnearby",
	}
	return names
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"scene-get-payment-cards": "/Account/CCWebConnect/GetPaymentCards",
		"showtimes":               "/prod/cpx/theatrical/api/v1/showtimes",
		"theatres":                "/prod/cpx/theatrical/api/v1/theatres",
		"theatres-playingnearby":  "/prod/cpx/theatrical/api/v1/theatres/playingnearby",
	}
