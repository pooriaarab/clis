...
// cineplex-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"cineplex-pp-cli/internal/learn/entities"
	"cineplex-pp-cli/internal/learn/lookups"
	"cineplex-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "CINEPLEX_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("cineplex-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  cineplex-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  cineplex-pp-cli recall "<question>" --agent
  cineplex-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cineplex-pp-cli learnings list --agent
  cineplex-pp-cli learnings list --query "<substring>"
  cineplex-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  cineplex-pp-cli learnings forget "<question>" --resource <id>
  cineplex-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  cineplex-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  cineplex-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
