...
Run sync first to populate the local search index.`,
  ...
  cineplex-pp-cli search "status"

  # Force local search explicitly
  cineplex-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  cineplex-pp-cli search "status" --type account --data-source local
  # JSON output for piping
  cineplex-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cineplex-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "concessions":
				results, err = db.SearchConcessions(query, limit)
			case "movies":
				results, err = db.SearchMovies(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchConcessions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search concessions failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchMovies(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search movies failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
