...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CINEPLEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CINEPLEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cineplex-pp-cli/feedback")
...
When ` + "`CINEPLEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CINEPLEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cineplex-pp-cli feedback list
  cineplex-pp-cli feedback list --limit 5
  cineplex-pp-cli feedback list --json`,
