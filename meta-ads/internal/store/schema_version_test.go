...
// TestMigrate_AddsColumnsOnUpgrade_ActAdAccountId verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_ActAdAccountId(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "act_ad_account_id" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("act_ad_account_id")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from act_ad_account_id after migrate", want)
		}
	}
}
