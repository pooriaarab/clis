...
type Config struct {
	...
	MetaAdsBearerAuth string          `toml:"ads_bearer_auth"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("META_ADS_BEARER_AUTH"); v != "" {
		cfg.MetaAdsBearerAuth = v
		cfg.markEnvOverride("MetaAdsBearerAuth")
		cfg.AuthSource = "env:META_ADS_BEARER_AUTH"
		cfg.CredentialSource = "env:META_ADS_BEARER_AUTH"
	}
	...
	if v := strings.TrimSpace(os.Getenv("META_ADS_AD_ACCOUNT_ID")); v != "" {
		cfg.TemplateVars["ad_account_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["ad_account_id"] = "ad_account_id_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("META_ADS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "meta-ads-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.MetaAdsBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:META_ADS_BEARER_AUTH"
		}
		return "Bearer " + c.MetaAdsBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		MetaAdsBearerAuth: c.MetaAdsBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.MetaAdsBearerAuth = creds.MetaAdsBearerAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.MetaAdsBearerAuth = ""
	delete(c.envOverrides, "MetaAdsBearerAuth")
	c.updateFileConfigField("MetaAdsBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MetaAdsBearerAuth"] {
			out.MetaAdsBearerAuth = c.fileConfig.MetaAdsBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MetaAdsBearerAuth":
		c.fileConfig.MetaAdsBearerAuth = c.MetaAdsBearerAuth
	}
}
