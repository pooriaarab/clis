...
func newLearningsCandidatesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "candidates",
		Short: "List improvement candidates awaiting review",
...
alter command behavior or recall's verified results on their own.`,
		Example: `# review pending campaign candidates before approving
meta-ads candidates --class campaign --status pending --limit 15

# include tombstoned (rejected) candidates while auditing decision history
meta-ads candidates --status rejected --tombstones --db staging.sqlite`,
...
meta-ads candidates --status rejected --tombstones --db staging.sqlite`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"meta-ads-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
