...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  meta-ads-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("META_ADS_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "META_ADS_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "META_ADS_BEARER_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'meta-ads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
