...
import (
	...
	"meta-ads-pp-cli/internal/learn/lookups"
...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "sync",
		Short: "Sync data locally for offline search",
...
Resource scoping:
  ...
  from a prior sync.`,
		Example: `# quick incremental sync of campaigns/ad sets since last run
meta-ads sync --resources campaigns,adsets --since 2026-07-10

# full resync with low concurrency to avoid rate limits, strict mode to fail loud on errors
meta-ads sync --full --concurrency 2 --strict

# scoped sync for one ad account param, skip pruning deleted records
meta-ads sync --param act_123456789 --no-prune --max-pages 5`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "meta-ads-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "act-ad-account-id":
		return db.UpsertActAdAccountId(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"act-ad-account-id": "/act_{ad_account_id}/adcreatives",
	}
...
var reconcileTypedTables = map[string]string{
	"act-ad-account-id": "act_ad_account_id",
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "act-ad-account-id\x00/act_{ad_account_id}/insights":
		return []string{"data"}
	case "act-ad-account-id\x00/act_{ad_account_id}/adcreatives":
		return []string{"data"}
	case "act-ad-account-id\x00/act_{ad_account_id}/adsets":
		return []string{"data"}
	case "act-ad-account-id\x00/act_{ad_account_id}/ads":
		return []string{"data"}
	case "act-ad-account-id\x00/act_{ad_account_id}/campaigns":
		return []string{"data"}
	case "act-ad-account-id\x00/act_{ad_account_id}/customaudiences":
		return []string{"data"}
	}
	return nil
}
