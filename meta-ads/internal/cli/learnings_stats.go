...
func newLearningsStatsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "stats",
		Short: "Show learning loop effectiveness stats",
...
local-only: events never leave this machine.`,
		Example: `# quick health check on sync/candidate counts
meta-ads stats

# same check against staging db before promoting a change
meta-ads stats --db staging.sqlite`,
