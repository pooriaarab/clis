...
func newProfileCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "profile",
		Short: "Save and reuse named flag sets",
...
captured: they control profile/config resolution and would never apply).`,
		Example: `# view current account profile the agent uses for context
meta-ads profile

# update profile description after account strategy shifted
meta-ads profile --description "B2B SaaS, prioritize lead gen over awareness"`,
