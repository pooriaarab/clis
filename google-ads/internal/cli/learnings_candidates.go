...
func newLearningsCandidatesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "candidates",
		Short: "Review suggested improvements waiting for approval",
...
alter command behavior or recall's verified results on their own.`,
		Example: `# list candidates flagged as anomalies, still open
google-ads candidates --class anomaly --status open --limit 20

# check candidates against specific local db during debugging
google-ads candidates --db ./learn.sqlite

# review tombstoned candidates before permanent cleanup
google-ads candidates --tombstones`,
...
google-ads candidates --tombstones`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"google-ads-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
