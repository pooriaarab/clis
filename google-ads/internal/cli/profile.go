...
func newProfileCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "profile",
		Short: "Save flag presets for reuse",
...
captured: they control profile/config resolution and would never apply).`,
		Example: `# check current profile config
google-ads profile

# update profile description after account strategy change
google-ads profile --description "ecommerce, focus on ROAS over volume"`,
