...
func newTeachPlaybookCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "teach-playbook",
		Short: "Save a reusable playbook with notes for similar queries",
...
agents can record playbooks without a file on disk.`,
		Example: `# register new playbook for recurring budget-check query
google-ads teach-playbook --query "budget pacing check" --playbook-file ./budget_pacing.json --notes "run weekly"

# append a follow-up note to existing playbook entry
google-ads teach-playbook --query "budget pacing check" --add-note "works for MCC accounts too" --db ./learn.sqlite`,
