...
// google-ads-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"google-ads-pp-cli/internal/learn/entities"
	"google-ads-pp-cli/internal/learn/lookups"
	"google-ads-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "GOOGLE_ADS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("google-ads-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "teach",
		Short: "Remember a query so it's recognized next time",
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `# teach system a resource query pattern from a known good query
google-ads teach --query "SELECT campaign.id FROM campaign" --resource campaign --resource-type standard

# feed a full playbook json with notes, skip validation for draft entry
google-ads teach --playbook-json ./draft_playbook.json --playbook-notes "initial draft, needs review" --no-validate

# review low-confidence mismatches before promoting to canonical
google-ads teach --debug-mismatches --min-confidence 0.6 --limit 25 --db ./learn.sqlite`,
