...
func newSearchCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "search <query>",
		Short: "Search synced data or live results",
...
In local mode: searches locally synced data only.`,
		Example: `# find matching resource docs for 'campaign_budget'
google-ads search campaign_budget --type resource --limit 10

# search local db for cached term instead of hitting live API
google-ads search "target_roas" --db ./cache.sqlite`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v24/customers/{customerId}/googleAds:search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v24/customers/{customerId}/googleAds:search", map[string]any{
					"query":     query,
					"pageToken": "",
					"pageSize":  10,
				})
