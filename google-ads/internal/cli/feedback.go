...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "feedback [text]",
		Short: "Share feedback about this CLI",
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `# send quick feedback note about a bad recommendation
google-ads feedback "bid suggestion too aggressive for branded terms" --send

# pipe feedback text from editor buffer
cat notes.txt | google-ads feedback --stdin --send

# check last few feedback entries before adding new one
google-ads feedback --limit 5`,
