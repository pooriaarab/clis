...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  google-ads-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Run 'google-ads-pp-cli auth login' to re-authenticate."
				} else {
					authConfigured = true
			...
			if os.Getenv("GOOGLE_ADS_OAUTH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_ADS_OAUTH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GOOGLE_ADS_OAUTH_TOKEN")
			}
			...
			if os.Getenv("GOOGLE_ADS_LOGIN_CUSTOMER_ID") != "" {
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GOOGLE_ADS_LOGIN_CUSTOMER_ID")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "google-ads-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'google-ads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
