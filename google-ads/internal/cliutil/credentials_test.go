...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"GOOGLE_ADS_CONFIG",
		"GOOGLE_ADS_CONFIG_DIR",
		"GOOGLE_ADS_DATA_DIR",
		"GOOGLE_ADS_STATE_DIR",
		"GOOGLE_ADS_CACHE_DIR",
		"GOOGLE_ADS_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "google-ads-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "google-ads-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("GOOGLE_ADS_OAUTH_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "google-ads-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "google-ads-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "google-ads-pp-cli", "credentials.toml")
