...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			...
			// Google Ads REST requires these as HTTP headers on every call,
			// on top of the OAuth Bearer — distinct from the generic
			// single-credential auth model this client was generated for.
			// See .printing-press-patches/developer-token-header.md.
			if c.Config.GoogleAdsDeveloperToken != "" {
				req.Header.Set("developer-token", c.Config.GoogleAdsDeveloperToken)
			}
			if c.Config.GoogleAdsLoginCustomerId != "" {
				req.Header.Set("login-customer-id", c.Config.GoogleAdsLoginCustomerId)
			}
		}
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "google-ads-pp-cli/1.0.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "google-ads-pp-cli auth login")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.GoogleAdsOauthToken)
		addCredential(c.Config.GoogleAdsDeveloperToken)
		addCredential(c.Config.GoogleAdsLoginCustomerId)
	}
