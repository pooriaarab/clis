...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-analytics-solo-pp-cli doctor
  google-analytics-solo-pp-cli doctor --json
  google-analytics-solo-pp-cli doctor --fail-on warn
  google-analytics-solo-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "google-analytics-solo-pp-cli login"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GOOGLE_APPLICATION_CREDENTIALS")
				...
				authEnvInfo = append(authEnvInfo, "GOOGLE_APPLICATION_CREDENTIALS optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GOOGLE_ANALYTICS_DATA_OAUTH2C")
				if os.Getenv("GOOGLE_ANALYTICS_DATA_OAUTH2C") != "" {
					authEnvSet = append(authEnvSet, "GOOGLE_ANALYTICS_DATA_OAUTH2C")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GOOGLE_ANALYTICS_DATA_OAUTH2C") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_ANALYTICS_DATA_OAUTH2C")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_ANALYTICS_DATA_OAUTH2C optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "google-analytics-solo-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'google-analytics-solo-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'google-analytics-solo-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'google-analytics-solo-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'google-analytics-solo-pp-cli sync' to refresh."
	}
	return report
}
