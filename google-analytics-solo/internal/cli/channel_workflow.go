...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  google-analytics-solo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  google-analytics-solo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account-summaries", "accounts", "google-analytics-admin-properties"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  google-analytics-solo-pp-cli workflow status

  # Show status as JSON
  google-analytics-solo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("google-analytics-solo-pp-cli")
			}
