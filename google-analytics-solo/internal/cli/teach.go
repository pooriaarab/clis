...
// google-analytics-solo-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"google-analytics-solo-pp-cli/internal/learn/entities"
	"google-analytics-solo-pp-cli/internal/learn/lookups"
	"google-analytics-solo-pp-cli/internal/learn/patterns"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("google-analytics-solo-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  google-analytics-solo-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  google-analytics-solo-pp-cli recall "<question>" --agent
  google-analytics-solo-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-analytics-solo-pp-cli learnings list --agent
  google-analytics-solo-pp-cli learnings list --query "<substring>"
  google-analytics-solo-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  google-analytics-solo-pp-cli learnings forget "<question>" --resource <id>
  google-analytics-solo-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  google-analytics-solo-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  google-analytics-solo-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
