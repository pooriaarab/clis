...
var whichIndex = []whichEntry{
	{Command: "report", Description: "Run one report against every registered property at once and get per-property results.", Group: "Multi-property", WhyItMatters: "Reach for this when an agent needs the same metric across every Solo property in one call instead of six."},
	{Command: "compare", Description: "Rank all registered properties by a single metric over a date range.", Group: "Multi-property", WhyItMatters: "Use when the question is 'which property leads on metric X', not single-property detail."},
	{Command: "trend", Description: "Show how a metric moved over time using locally cached report runs, no re-query.", Group: "Local state that compounds", WhyItMatters: "Use for offline trend analysis after reports have been cached locally."},
	{Command: "report", Description: "Save a report spec by name and replay it without retyping dimensions and metrics.", Group: "Local state that compounds", WhyItMatters: "Use to replay common reports; agents can reference a named report instead of a full spec."},
	{Command: "properties add", Description: "Register numeric property IDs under friendly names once, then reference by name everywhere.", Group: "Multi-property", WhyItMatters: "Use once at setup so every later command can say --property solo-prod."},
	{Command: "metadata search", Description: "Full-text search the cached dimension/metric catalog to find exact API names.", Group: "Local state that compounds", WhyItMatters: "Use to find the exact dimension/metric API name before building a report."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  google-analytics-solo-pp-cli which "stale tickets"
  google-analytics-solo-pp-cli which "bottleneck"
  google-analytics-solo-pp-cli which --limit 1 "send message"
  google-analytics-solo-pp-cli which                                # list the full capability index`,
