...
// posthog-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"posthog-pp-cli/internal/learn/entities"
	"posthog-pp-cli/internal/learn/lookups"
	"posthog-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "POSTHOG_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("posthog-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  posthog-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  posthog-pp-cli recall "<question>" --agent
  posthog-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  posthog-pp-cli learnings list --agent
  posthog-pp-cli learnings list --query "<substring>"
  posthog-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  posthog-pp-cli learnings forget "<question>" --resource <id>
  posthog-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  posthog-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  posthog-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
