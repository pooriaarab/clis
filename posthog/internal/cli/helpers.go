...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: posthog-pp-cli auth set-token <token> or export POSTHOG_PERSONAL_APIKEY_AUTH=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
