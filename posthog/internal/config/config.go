...
type Config struct {
	...
	PosthogPersonalApikeyAuth string          `toml:"personal_apikey_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("POSTHOG_PERSONAL_APIKEY_AUTH"); v != "" {
		...
		cfg.markEnvOverride("PosthogPersonalApikeyAuth")
		cfg.AuthSource = "env:POSTHOG_PERSONAL_APIKEY_AUTH"
		cfg.CredentialSource = "env:POSTHOG_PERSONAL_APIKEY_AUTH"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("POSTHOG_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "posthog-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.PosthogPersonalApikeyAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:POSTHOG_PERSONAL_APIKEY_AUTH"
		}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		PosthogPersonalApikeyAuth: c.PosthogPersonalApikeyAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PosthogPersonalApikeyAuth = creds.PosthogPersonalApikeyAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.PosthogPersonalApikeyAuth = ""
	delete(c.envOverrides, "PosthogPersonalApikeyAuth")
	c.updateFileConfigField("PosthogPersonalApikeyAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PosthogPersonalApikeyAuth"] {
			out.PosthogPersonalApikeyAuth = c.fileConfig.PosthogPersonalApikeyAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PosthogPersonalApikeyAuth":
		c.fileConfig.PosthogPersonalApikeyAuth = c.PosthogPersonalApikeyAuth
	}
}
