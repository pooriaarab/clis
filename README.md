...
| [`yandex-direct/`](yandex-direct/README.md) | Thin unverified print: `reporting get` (POST JSON to the documented v501 reports endpoint) only; all other groups are listed in `--help` but exit with a not-implemented TODO | OAuth 2.0 access token | [App registration and API access](https://yandex.com/dev/direct/doc/start/register.html) |
...
"Auth-only stub" means the built binary exposes only `doctor` and `auth` (`status`, `login`, `set-token`) — the `accounts`/`campaigns`/`audiences`/`reporting` functions exist in source as an unwired template and are not reachable. "Thin unverified print" means the commands exist and hit documented endpoint paths as generic `--body` JSON passthrough, but were never verified end-to-end against a real account, unlike the big three above.
