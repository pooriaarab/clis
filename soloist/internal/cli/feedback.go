...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SOLOIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SOLOIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "soloist-pp-cli/feedback")
...
When ` + "`SOLOIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SOLOIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  soloist-pp-cli feedback list
  soloist-pp-cli feedback list --limit 5
  soloist-pp-cli feedback list --json`,
