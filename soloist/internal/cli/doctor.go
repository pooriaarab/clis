...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  soloist-pp-cli doctor
  soloist-pp-cli doctor --json
  soloist-pp-cli doctor --fail-on warn
  soloist-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: soloist-pp-cli auth set-token <token> or export SOLOIST_ID_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("SOLOIST_ID_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SOLOIST_ID_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SOLOIST_ID_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "soloist-pp-cli", suggestion)
						} else {
