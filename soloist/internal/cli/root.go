...
type rootFlags struct {
	...
	// invocation. Mirrors the SOLOIST_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "soloist-pp-cli",
		Short: `Manage your Solo websites (draft + published), custom domains, and share invites directly against Firestore, the same backend the soloist.ai designer uses.`,
		Long: `Manage your Solo websites (draft + published), custom domains, and share invites directly against Firestore, the same backend the soloist.ai designer uses.

Add --agent to any command for JSON output + non-interactive mode.
Run 'soloist-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("soloist-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'soloist-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDraftWebsitesCmd(flags))
