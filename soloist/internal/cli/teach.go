...
// soloist-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"soloist-pp-cli/internal/learn/entities"
	"soloist-pp-cli/internal/learn/lookups"
	"soloist-pp-cli/internal/learn/patterns"
	"soloist-pp-cli/internal/store"
)
...
const noLearnEnvVar = "SOLOIST_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("soloist-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  soloist-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  soloist-pp-cli recall "<question>" --agent
  soloist-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  soloist-pp-cli learnings list --agent
  soloist-pp-cli learnings list --query "<substring>"
  soloist-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  soloist-pp-cli learnings forget "<question>" --resource <id>
  soloist-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  soloist-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  soloist-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
