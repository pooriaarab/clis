...
type Config struct {
	...
	SoloistIdToken   string          `toml:"id_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("SOLOIST_ID_TOKEN"); v != "" {
		cfg.SoloistIdToken = v
		cfg.markEnvOverride("SoloistIdToken")
		cfg.AuthSource = "env:SOLOIST_ID_TOKEN"
		cfg.CredentialSource = "env:SOLOIST_ID_TOKEN"
	}
	...
	if v := os.Getenv("SOLOIST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SOLOIST_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "soloist-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SoloistIdToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SOLOIST_ID_TOKEN"
		}
		return "Bearer " + c.SoloistIdToken
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SoloistIdToken: c.SoloistIdToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.SoloistIdToken = creds.SoloistIdToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SoloistIdToken = ""
	delete(c.envOverrides, "SoloistIdToken")
	c.updateFileConfigField("SoloistIdToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SoloistIdToken"] {
			out.SoloistIdToken = c.fileConfig.SoloistIdToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SoloistIdToken":
		c.fileConfig.SoloistIdToken = c.SoloistIdToken
	}
}
